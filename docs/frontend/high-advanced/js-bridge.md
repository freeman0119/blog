...
| **特性**     | **url schema 方式**                                                                                                                                                                                                                                                            | **注入 js 上下文方式**                                                                                                                                                                                                                                    |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **原理**     | URL Schema 方式是一种基于 URL 的通信方案，通过约定特定的 URL 格式，在 JavaScript 和原生应用程序之间进行通信。                                                                                                                                                                  | JavaScript 注入方式是通过 WebView 提供的接口方法，将 native 的接口注入到 WebView 中执行，从而实现 JavaScript 调用原生方法，以及原生调用 JavaScript 方法的功能。                                                                                           |
| **实现步骤** | 1. 在 Web 页面中构造带有特定格式的 URL，包括调用的方法名和参数，并可以附带回调函数。<br>2. 原生应用程序拦截并解析 Web 页面加载的 URL，执行相应的原生方法，并将结果通过 URL 返回给 JavaScript。<br>3. JavaScript 接收到原生方法执行的结果，并进行相应的处理，包括调用回调函数。 | 1. JavaScript 注入方式是通过 WebView 提供的接口方法，将 native 的接口注入到 WebView 中执行，从而实现 JavaScript 调用原生方法，以及原生调用 JavaScript 方法的功能                                                                                          |
| **优点**     | 1. 实现简单，无需额外的技术支持，适用于基本的原生与 Web 交互需求。<br>2. 支持双向通信，并且支持在原生调用 JavaScript 方法时传递参数。                                                                                                                                          | 1. 通过 webview 的接口，将 native 的接口注入的 window 对象中 <br>2. web 端通过 window 对象中对应的方法，实现调用 native 方法的功能，同时可以传入回调 id，实现回调的相关功能。<br>3. native 可以通过 webview 的接口，直接调用 web 端中 window 对象下的方法 |
| **缺点**     | 1. 回调函数需要通过 URL 参数的方式传递，参数较多时不够灵活。<br>2. 在 Web 页面中暴露了原生方法的调用方式，安全性较低。                                                                                                                                                         | 1.需要较多的原生代码来处理 JavaScript 方法的调用和结果的返回。 <br> 2.在 Android 平台，需要 Android 4.4（API level 19）及以上版本支持，iOS 平台需要 iOS 8.0 及以上版本支持。                                                                              |
